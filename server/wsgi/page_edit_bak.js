...
//	refBox.setAttribute("readonly", 'yes');
	refBox.setAttribute("style","position:absolute;top:15%;left:4%;min-width:450px;max-width:450px;min-height:370px;max-height:370px;text-align:justify;color:#000;list-style:none;");
	...
	editBox.setAttribute("style","position:absolute;top:15%;left:51%;min-width:450px;max-width:450px;min-height:370px;max-height:370px;font-size:15;text-align:justify;color:#000;");
...
//	    console.log(element.parentNode.id);
	    element.setAttribute('id', 'a11ypi');
	    popupControl.openAt(element, popX, popY);
	};
...
//	    console.log(selectedElement.parentNode.textContent);
	    ref = document.getElementById('reference');
//	    ref.value = selectedElement.innerHTML;
	    children = selectedElement.childNodes;
	    console.log(children);
	    for (var i=0; i<children.length; i++) {
		if(i == 0 ) {
		    ref.innerHTML = '&lt' + selectedElement.parentNode.tagName + '&gt';
		    topTag = document.createElement(selectedElement.parentNode.tagName);
		    topTag.textContent = children[i].textContent;
		    ref.appendChild(topTag);
		    ...
		    bottomTag = document.createElement(selectedElement.parentNode.tagName);
		    bottomTag.textContent = children[i].textContent;
		    ref.appendChild(bottomTag);
		    ref.innerHTML += '&lt/' + selectedElement.parentNode.tagName + '&gt';
		} else {
		    ref.innerHTML += '&lt' + children[i].tagName + '&gt';
		    midTag = document.createElement(selectedElement.parentNode.tagName);
		    midTag.textContent = children[i].textContent;
		    ref.appendChild(midTag);
		    ref.innerHTML += '&lt/' + children[i].tagName + '&gt';
		}
	    }
	    ref.addEventListener('click', editElement, false);
	    function editElement(e) {
		console.log(e.explicitOriginalTarget.textContent);
	    }
...
//		    return false;
	...
	this.apply = function apply(command) {
		...
		command.previousData = document.getElementById("reference").value;
